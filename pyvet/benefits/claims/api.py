...
from pyvet.creds import API_URL, BEARER_TOKEN
...
def get_claims(
    ...
    if is_representative:
        ...
        headers["X-VA-First-Name"] = first_name
        headers["X-VA-Last-Name"] = last_name
        headers["X-VA-Birth-Date"] = birth_date

    claims_url = BENEFITS_INTAKE_URL + f"claims"
    try:
        r = session.get(claims_url, headers=headers)
        r.raise_for_status()
        r = r.json()
        return r
...
def get_claim(
    ...
    if is_representative:
        ...
        headers["X-VA-Birth-Date"] = birth_date
    try:
        r = session.get(claim_url, headers=headers)
        r.raise_for_status()
        r = r.json()
        return r
    except requests.exceptions.RequestException as e:
